...
{
	if (driverStat.formed == 1 && sampleStat.formed == 1){
		ready.dutyCycle = 250;
	} else {
		ready.dutyCycle = 0;
	}
...
int main(int argc, char **argv)
{
	...
	ready.dutyCycle = 0;
	error.dutyCycle = 0;
	ready.file.open("/sys/class/hwmon/hwmon1/device/pwm1", std::ios::out | std::ios::binary);
	error.file.open("/sys/class/hwmon/hwmon1/device/pwm2", std::ios::out | std::ios::binary); //| std::ios::trunc);
